...
const debug = require('debug')('sectord17-game:connection-manager-udp');
    ...
    _onLoginMsg(rinfo, loginMsg) {
        return this.playerManager
            ...
            .then(player => {
                ...
                debug(`Player <${player.token}> connected via UDP`);

                return player;
            })
            .then(player => this.playerManager.tryToMakeConnected(player))
            .then(player => this.connectionHandlers.set(player.connectionHandlerUdp.key(), player.connectionHandlerUdp))
