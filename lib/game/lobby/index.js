...
class Lobby {
    ...
    _tryStartGame() {
        const readyPlayers = this.all()
            .filter(playerInfo => playerInfo.ready);

        if (readyPlayers.length < 2 || readyPlayers.length < this.all().length) {
            return false;
        }

        readyPlayers.forEach(player => player.communicationHandler.sendViaTcp(FlatBuffersHelper.roomMsg.gameStart()));
        winston.log("Game started!");
